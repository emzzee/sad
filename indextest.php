...
if ($stmt->rowCount() > 0)
    ...
    <?php if (!empty($row)) : ?>
        <div class="container mt-5">
            <div class="row border rounded p-2 shadow-lg">
                <!-- Image Column -->
                <div class="col-md-4 text-center">
                    ...
                    <!-- Other elements like edit and delete buttons -->
                    <?php if (user('user_id') == $row['user_id']) : ?>
                    <a href="amouser-edit.php">
                                        <button class="mx-auto m-1 btn-sm btn btn-primary">Edit</button>
                                    </a>
                                    
                                        <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#deleteUserModal" onclick="setUserIdToDelete(<?= $_SESSION['USER']['user_id']; ?>)">Delete</button>
                                    
                </div>
                <!-- User Details Column -->
                <div class="col-md-8">
                    <div class="h2">User Profile</div>
                    <form action="amouser-edit" class="post">
                        <!-- User details form -->
                        <table class="table table-striped">
                                    ...
                                    <tr>
                                        ...
                                        <td><input id="contact" value="<?php echo esc($row['mobile_number']) ?>" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" name="contact" type="text" minlength="11" maxlength="11"></td>
                                    </tr>
                                    <tr>
                                        <th><i class="bi bi-person-circle"></i> First name</th>
                                        <td><input type="text" name="first_name" id="first_name" value="<?php echo esc($row['first_name']) ?>"></td>
                                    </tr>
                                    <tr>
                                        <th><i class="bi bi-person-square"></i> Last name</th>
                                        <td><input type="text" name="last_name" id="last_name" value="<?php echo esc($row['last_name']) ?>"></td>
                                    </tr>
                                    <tr>
                                        <th><i class="bi bi-geo-alt-fill"></i> Address</th>
                                        <td><input type="text" name="address" id="address" value="<?php echo esc($row['address']) ?>"></td>
                                    </tr>
                                    <tr>
                                        <th><i class="bi bi-calendar"></i> Birthdate</th>
                                        <td><input type="date" name="birthdate" id="birthdate" value="<?php echo esc($row['birthdate']) ?>"></td>
                                    </tr>
                                    <tr>
                                        <th><i class="bi bi-person-fill"></i> Role</th>
                                        <td><?php echo $role[$row['user_role']] ?></td>
                                    </tr>
                                </table>
                    </form>
                </div>
